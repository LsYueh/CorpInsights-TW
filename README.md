...
> 💡 **執行模式與轉檔說明**
> 
> * **批量執行 (`--report all`) 預設排除 `現金流量表 (T163SB20)`**：
>   因該資料來源取得較繁瑣，需手動人工介入處理，且現階段資料實用性較低，故批量批次時預設不執行。
> * **支援單獨呼叫**：
>   若有特定需求需轉換現金流量表，請於執行命令時指定參數 `--report T163SB20` 單獨發起轉檔作業
...
[資產擴張版](/docs/misc/盈餘再投資率/資產擴張版.md)  
[盈餘保留版](/docs/misc/盈餘再投資率/盈餘保留版.md)  
