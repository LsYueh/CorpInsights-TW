...
### 6. 資料日期 (`--skip-date-check`)

控制是否跳過交易日的自動校正：

* 預設（未帶此參數）：系統會自動進行交易日校正。若指定的 --date 為假日或非交易日，會自動往前調整至最近營業日。
* 帶入此參數：強制跳過交易日校正，保持帶入的原始日期執行。（注意：JSON 內部的資料日期比對機制依然會正常執行）。
