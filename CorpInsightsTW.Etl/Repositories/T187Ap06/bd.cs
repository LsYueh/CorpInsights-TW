...
public class BdRepository(string connectionString)
{
    ...
    /// <summary>
    /// 批次寫入或更新綜合損益表-證券期貨業 (t187ap06_bd)
    /// </summary>
    public async Task UpsertAsync(IEnumerable<BdDto> dtos, CancellationToken cancellationToken = default)
    {
        if (dtos == null || !dtos.Any()) return;

        const string sql = @"
            INSERT INTO t187ap06_bd (
                year,
                quarter,
                listing_status,
                company_code,
            ...
            ON DUPLICATE KEY UPDATE
                ...
                expenses_and_costs            = VALUES(expenses_and_costs),
