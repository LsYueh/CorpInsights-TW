    ...
    LocalRawDataStorage storage)
{
    ...
    public async Task FetchDataAsync(FetchContext context, CancellationToken ct, int indentLevel = 0)
    {
        ...
        string targetUrl = GetTargetUrl(context, indentLevel);
    ...
    public string GetTargetUrl(FetchContext context, int indentLevel = 0)
    {
        string indent = GetIndent(indentLevel);
        ...
        string baseUrl = context.Market switch
        {
            StockMarket.TWSE => _config.TwseRootUrl.TrimEnd('/'), // 確保 RootUrl 結尾乾淨
            StockMarket.TPEX => _config.TpexRootUrl.TrimEnd('/'),
            _ => throw new NotSupportedException($"未支援的市場分類: '{context.Market}'")
        };
        ...
        {
            StockMarket.TWSE => $"{baseUrl}/opendata/{file}",
            StockMarket.TPEX => $"{baseUrl}/mopsfin_{file}",
            _ => throw new NotSupportedException($"未支援的市場分類: '{context.Market}'")
        };
