CREATE TABLE IF NOT EXISTS `t187ap06_fh` (
    ...
    -- 2. 金控業特有營業淨收益項目
    `net_interest_income`                         DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '利息淨收益',
    `net_other_income`                            DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '其他收益及費損淨額',
    `non_interest_income`                         DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '利息以外淨收益',
    `net_revenue`                                 DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '淨收益',

    -- 3. 金控與保險特有成本準備與費用項目
    `bad_debt_and_provision`                      DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '呆帳費用、承諾及保證責任準備提存',
    `net_change_insurance_liab`                   DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '保險負債準備淨變動',
    `operating_expenses`                          DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '營業費用',

    -- 4. 稅前與稅後淨利項目
    `income_before_tax`                           DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '繼續營業單位稅前損益',
    `income_tax`                                  DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '所得稅（費用）利益',
    ...
    `net_income`                                  DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '本期稅後淨利（淨損）',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='公司綜合損益表-金控業';