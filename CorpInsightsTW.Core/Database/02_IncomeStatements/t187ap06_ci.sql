CREATE TABLE IF NOT EXISTS `t187ap06_ci` (
    ...
    -- 2. 營業收入、成本與毛利項目
    `revenue`                                     DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '營業收入',
    `costs`                                       DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '營業成本',
    `bio_assets_initial_gain_loss`                DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '原始認列生物資產及農產品之利益（損失）',
    `bio_assets_fv_gain_loss`                     DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '生物資產當期公允價值減出售成本之變動利益（損失）',
    `gross_profit`                                DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '營業毛利（毛損）',
    `unrealized_gains_losses`                     DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '未實現銷貨（損）益',
    `realized_gains_losses`                       DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '已實現銷貨（損）益',
    `net_gross_profit`                            DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '營業毛利（毛損）淨額',

    -- 3. 營業費用與營業利益
    `expenses`                                    DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '營業費用',
    `net_other_income`                            DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '其他收益及費損淨額',
    `operating_income`                            DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '營業利益（損失）',
    `non_operating_income`                        DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '營業外收入及支出',
    ...
    -- 5. 其他綜合損益項目
    `other_comprehensive_income`                  DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '其他綜合損益（淨額）',
    ...
    -- 6. 損益歸屬項目
    `net_income_parent`                           DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '淨利（淨損）歸屬於母公司業主',
    `net_income_predecessor`                      DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '淨利（淨損）歸屬於共同控制下前手權益',
    `net_income_nci`                              DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '淨利（淨損）歸屬於非控制權益',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='公司綜合損益表-一般業';