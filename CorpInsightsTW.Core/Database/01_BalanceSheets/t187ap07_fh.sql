CREATE TABLE IF NOT EXISTS `t187ap07_fh` (
    ...
    `due_from_central_bank_and_financial_peers`   DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '存放央行及拆借金融同業',
    ...
    `financial_assets_at_amortized_cost`          DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '按攤銷後成本衡量之債務工具投資',
    `derivative_financial_assets_for_hedging`     DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '避險之金融資產 / 避險之衍生金融資產',
    `reverse_repo`                                DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '附賣回票券及債券投資',
    `receivables`                                 DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '應收款項－淨額',
    `current_tax_assets`                          DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '本期所得稅資產 / 當期所得稅資產',
    `assets_held_for_sale`                        DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '待出售資產－淨額',
    `discounts_and_loans`                         DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '貼現及放款－淨額',
    `reinsurance_contract_assets`                 DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '保險合約資產及再保險合約資產－淨額',
    `investments_using_equity_method`             DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '採用權益法之投資－淨額',
    `restricted_assets`                           DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '受限制資產－淨額',
    `other_financial_assets`                      DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '其他金融資產－淨額',
    `investment_property`                         DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '投資性不動產－淨額',
    `property_and_equipment`                      DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '不動產及設備－淨額',
    `right_of_use_assets`                         DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '使用權資產－淨額',
    `intangible_assets`                           DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '無形資產－淨額',
    `deferred_tax_assets`                         DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '遞延所得稅資產',
    `other_assets`                                DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '其他資產－淨額',
    `total_assets_amount`                         DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '資產總計 / 資產總額',

    -- 3. 金控業負債類項目
    `deposits_from_central_bank_and_financial_peers` DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '央行及金融同業存款',
    `due_to_central_bank_and_financial_peers`     DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '央行及同業融資',
    `financial_liabs_at_fvtpl`                    DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '透過損益按公允價值衡量之金融負債',
    `derivative_financial_liabs_for_hedging_net`  DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '避險之金融負債 / 避險之衍生金融負債',
    `repo_liabs`                                  DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '附買回票券及債券負債',
    `commercial_paper_payable_net`                DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '應付商業本票－淨額',
    `payables`                                    DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '應付款項',
    `current_tax_liabs`                           DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '本期所得稅負債 / 當期所得稅負債',
    `liabs_related_to_assets_held_for_sale`       DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '與待出售資產直接相關之負債',
    `deposits_and_remittances`                    DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '存款及匯款',
    `bonds_payable`                               DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '應付債券',
    `other_borrowings`                            DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '其他借款',
    `preferred_stock_liabs`                       DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '特別股負債',
    `provisions_for_liabs`                        DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '負債準備',
    ...
    `total_liabs_amount`                          DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '負債總計 / 負債總額',
    ...
    `equity_attributable_to_owners_of_parent`     DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '歸屬於母公司業主之權益',
    ...
    `total_equity_amount`                         DECIMAL(20, 2)  NOT NULL DEFAULT 0.00 COMMENT '權益總計 / 權益總額',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='公司資產負債表-金控業';